package org.flib.xdstorage.sqlstorage.sql.builders.search;

import org.flib.xdstorage.sqlstorage.sql.XdStorageSQLBuilderName;
import org.flib.xdstorage.sqlstorage.sql.builders.XdStorageAbstractSQLBuilder;
import org.flib.xdstorage.utils.XdStorageClassInfo;
import org.flib.xdstorage.utils.XdStorageObjectIdField;

public class XdStorageSelectInsertSearchIndexByTransactionIdSQLBuilder extends XdStorageAbstractSQLBuilder {
    @Override
    public String build(final Object... values) {
        final XdStorageSearchIndexTableName indexTableName = cast(values[0]);
        final String table = cast(values[1]);
        final String prevStateTable = cast(values[2]);
        final XdStorageClassInfo clInfo = cast(values[3]);

        final String sqlId = buildSQLId(indexTableName, table);

        String sql = getSQL(sqlId);
        if (sql == null) {
            sql = build(indexTableName, table, prevStateTable, clInfo);
            putSQL(sqlId, sql);
        }
        return sql;
    }

    private String buildSQLId(final XdStorageSearchIndexTableName indexTableName, final String table) {
        return XdStorageSQLBuilderName.SelectInsertSearchIndexByTransaction.name() + table + indexTableName;
    }

    public String build(final XdStorageSearchIndexTableName indexTableName, final String table,
                        final String prevStateTable, final XdStorageClassInfo clInfo) {
        final StringBuilder sb = new StringBuilder("INSERT INTO ");

        final XdStorageObjectIdField idField = clInfo.getIdField();

        sb.append(table).append('_').append(indexTableName.getNameTableSuffix()).append(" (");
        switch(indexTableName) {
            case OBJECTS_REFERENCES:
                sb.append(idField.getName()).append(", rtable ").append(", rdatasource ").append(", txname ");
                break;
            case OBJECTS_FIELDS:
                sb.append(idField.getName()).append(", field_name ").append(", value_class_name ")
                        .append(", value_as_string ").append(", txname ");
                break;
            case OBJECTS_CHILDREN_FIELDS:
                sb.append(idField.getName()).append(' ').append(", child_class_name ")
                        .append(", child_field_name ").append(", id_class_name ")
                        .append(", id_as_string ").append(", field_name ")
                        .append(", value_class_name ").append(", value_as_string ")
                        .append(", txname ");
        }
        sb.append(")");

        sb.append(" SELECT ");
        switch(indexTableName) {
            case OBJECTS_REFERENCES:
                sb.append(idField.getName()).append(", rtable ").append(", rdatasource ").append(", NULL ");
                break;
            case OBJECTS_FIELDS:
                sb.append(idField.getName()).append(", field_name ").append(", value_class_name ")
                        .append(", value_as_string ").append(", NULL ");
                break;
            case OBJECTS_CHILDREN_FIELDS:
                sb.append(idField.getName()).append(' ').append(", child_class_name ")
                        .append(", child_field_name ").append(", id_class_name ")
                        .append(", id_as_string ").append(", field_name ")
                        .append(", value_class_name ").append(", value_as_string ")
                        .append(", NULL ");
        }
        sb.append(" FROM ").append(prevStateTable).append('_').append(indexTableName.getNameTableSuffix());
        sb.append(" WHERE txname = ?");

        return sb.toString();
    }
}
