...
public class XdStorageCrossDatasourceReferenceResourceSecondPhaseCommitController {
    ...
    public XdStorageCrossDatasourceReferenceResourceSecondPhaseCommitController(final Object resourceId, final XdStorageServicesLocator services, final XdStorageSQLResourceNamingService namingService,
    ...
    public void initQueries() throws XdStorageException {
        ...
        cleanQuery = factory.getBuilder(XdStorageSQLBuilderName.UpdateCrossDatasourceReferenceByTransaction).build(table);
        cleanPrevStateQuery = factory.getBuilder(XdStorageSQLBuilderName.DeleteCrossDatasourceReferenceByTransaction).build(prevStateTable);
    }
    ...
    public void unlockObjects(final XdStorageSQLProcessor processor, final XdStorageTransaction transaction) {
        final XdStorageSQLCommand cleanCommand = new XdStorageSQLCommand(XdStorageSQLBuilderName.UpdateCrossDatasourceReferenceByTransaction.priority, resourceId, cleanQuery, helper);
        cleanCommand.addParameter(Boolean.TRUE);
        cleanCommand.addParameter(transaction.getTransactionId());

        processor.pushCommand(transaction, cleanCommand);

        final XdStorageSQLCommand cleanPrevStateCommand = new XdStorageSQLCommand(XdStorageSQLBuilderName.DeleteCrossDatasourceReferenceByTransaction.priority, resourceId, cleanPrevStateQuery, helper);
        cleanPrevStateCommand.addParameter(Boolean.TRUE);
